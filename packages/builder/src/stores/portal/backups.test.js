...
describe('backups store', () => {
  ...
  describe('selectBackup', () => {
    ...
    it('sets the state with the selected backup', (ctx) => {
      ...
      expect(ctx.writableReturn.update.calls[0][0]({})).toEqual({ selectedBackup: ctx.backupId })
    })
  })
  ...
  describe('deleteBackup', () => {
    beforeEach(async (ctx) => {
      ctx.appId = 'appId'
      ctx.backupId = 'backupId'
      ctx.value = await ctx.returnedStore.deleteBackup({ appId: ctx.appId, backupId: ctx.backupId })
    })

    it('calls and returns the API deleteBackup method', (ctx) => {
      expect(API.deleteBackup).toHaveBeenCalledTimes(1)
      expect(API.deleteBackup).toHaveBeenCalledWith({ appId: ctx.appId, backupId: ctx.backupId })
  ...
  describe('restoreBackup', () => {
    beforeEach(async (ctx) => {
      ...
      ctx.value = await ctx.returnedStore.restoreBackup({ appId: ctx.appId, backupId: ctx.backupId, name: ctx.$name })
    })

    it('calls and returns the API restoreBackup method', (ctx) => {
      expect(API.restoreBackup).toHaveBeenCalledTimes(1)
      expect(API.restoreBackup).toHaveBeenCalledWith({ appId: ctx.appId, backupId: ctx.backupId, name: ctx.$name })
  ...
  describe('updateBackup', () => {
    beforeEach(async (ctx) => {
      ...
      ctx.backupId = 'backupId'
      ctx.$name = 'name' // `name` is used by some sort of internal ctx thing and is readonly
      ctx.value = await ctx.returnedStore.updateBackup({ appId: ctx.appId, backupId: ctx.backupId, name: ctx.$name })
    })

    it('calls and returns the API updateBackup method', (ctx) => {
      expect(API.updateBackup).toHaveBeenCalledTimes(1)
      expect(API.updateBackup).toHaveBeenCalledWith({ appId: ctx.appId, backupId: ctx.backupId, name: ctx.$name })
