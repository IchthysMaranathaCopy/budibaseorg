...
vi.mock('api', () => {
  return { API: { checkImportComplete: vi.fn(), getEnvironment: vi.fn(), getSystemStatus: vi.fn(), getChecklist: vi.fn() } }
})
...
describe('admin store', () => {
  ...
  it('inits the writable store with the default config', (ctx) => {
  ...
  describe('init method', () => {
    beforeEach(async (ctx) => {
      ...
      API.getEnvironment.mockReturnValue({ multiTenancy: true, cloud: true, disableAccountPortal: true, accountPortalUrl: 'url', isDev: true })
    ...
    describe('getEnvironment', () => {
      ...
      it('adds the environment to the store', (ctx) => {
        ...
        expect(ctx.writableReturn.update.calls[1][0]({ foo: 'foo' })).toEqual({ foo: 'foo', multiTenancy: true, cloud: true, disableAccountPortal: true, accountPortalUrl: 'url', isDev: true })
      })
    })
    ...
    describe('system status', () => {
      ...
      describe('cloud with healthy admin status', () => {
        ...
        it('getSystemStatus', (ctx) => {
          ...
          expect(API.getEnvironment).toHaveBeenCalledWith()
          expect(ctx.writableReturn.update.calls[2][0]({ foo: 'foo' })).toEqual({ foo: 'foo', status: 'status'})
        })
    ...
    describe('getEnvironment', () => {
      ...
      it('marks the store as loaded', (ctx) => {
        expect(ctx.writableReturn.update.calls[3][0]({ foo: 'foo' })).toEqual({ foo: 'foo', loaded: true})
      })
    })
  });
  ...
  describe('checkImportComplete', () => {
    describe('import complete', () => {
      ...
      it('updates the store\'s importComplete parameter', (ctx) => {
        expect(ctx.writableReturn.update.calls[0][0]({ foo: 'foo' })).toEqual({ foo: 'foo', importComplete: true})
      })
    })
    ...
    describe('import not complete', () => {
      ...
      it('updates the store\'s importComplete parameter', (ctx) => {
        expect(ctx.writableReturn.update.calls[0][0]({ foo: 'foo' })).toEqual({ foo: 'foo', importComplete: false})
      })
    })
  })
  ...
  describe('unload', () => {
    ...
    it('sets the store\'s loaded parameter to false', (ctx) => {
      expect(ctx.writableReturn.update.calls[0][0]({ loaded: true })).toEqual({ loaded: false })
    })
  })
