...
describe('admin store', () => {
  ...
  describe('init', () => {
    ...
    it('inits the derived store with the initial writable store and an update function', (ctx) => {
  ...
  describe('wrapSaveDoc', () => {
    ...
    it('sets the state to loading', (ctx) => {
      expect(ctx.writableReturn.update.calls[0][0]({})).toEqual({ loading: true })
    })
    ...
    it('saves the operation', (ctx) => {
      expect(ctx.writableReturn.update.calls[1][0]({
        history: [],
        position: 0
      })).toEqual({
        history: [{
    ...
    it('sets the state to not loading', (ctx) => {
      ...
      expect(ctx.writableReturn.update.calls[2][0]({})).toEqual({ loading: false })
    })
  })
  ...
  describe('undo', () => {
    beforeEach(async (ctx) => {
      ...
      ctx.derivedReturn = { subscribe: vi.fn(), canUndo: true, history:  ctx.history, position: 1, loading: false }
    ...
    it('sets the state to the previous position', (ctx) => {
      expect(ctx.writableReturn.update.calls[1][0]({ history: [], position: 1})).toEqual({ history: [], position: 0 })
    })
    ...
    it('sets the state to not loading', (ctx) => {
      expect(ctx.writableReturn.update.calls[5][0]({})).toEqual({ loading: false })
    })
  ...
  describe('redo', () => {
    beforeEach(async (ctx) => {
      ctx.history = [{ type: Operations.Delete, doc: { _id: 1236, _rev: 'rev' } }]
      ctx.derivedReturn = { subscribe: vi.fn(), canRedo: true, history:  ctx.history, position: 0, loading: false }
    ...
    it('sets the state to the next position', (ctx) => {
      expect(ctx.writableReturn.update.calls[1][0]({ history: [], position: 0})).toEqual({ history: [], position: 1 })
    })
